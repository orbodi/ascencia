...
export function CurriculumPage() {
  ...
  const [createWeeks, setCreateWeeks] = useState(6);
  ...
  const createPlan = useMutation({
    mutationFn: () =>
      api<CurriculumPlan>("/admin/curriculum-plans", {
        method: "POST",
        body: JSON.stringify({
          academic_level_id: Number(createLevelId),
          semester: Number(createSemester),
          week_count: Number(createWeeks),
        }),
      }),
    onSuccess: (plan) => {
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            {isLoading ? (
              ...
              <p className="text-sm text-ink-soft">Aucun programme.</p>
            ) : (
          ...
          <Card className="p-5">
            <h2 className="mb-3 font-semibold">Nouveau programme</h2>
            <form className="space-y-3" onSubmit={onCreate}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Select
                  ...
                  onChange={(e) => setCreateSemester(Number(e.target.value))}
                >
                ...
                <Input
                  ...
                  max={52}
                  value={createWeeks}
                  onChange={(e) => setCreateWeeks(Number(e.target.value))}
                />
              </div>
          ...
          {!selected ? (
            ...
            <>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {Array.from({ length: selected.week_count }, (_, i) => i + 1).map(
                  (n) => {
                    const count =
              ...
              <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_100px_auto]">
                ...
                <div className="flex items-end">
                  <Button type="button" onClick={addItem} disabled={!addCourseId}>
                    Ajouter
                  </Button>
                </div>
              </div>
                ...
                <tbody>
                  {draftItems.length === 0 ? (
                    <tr>
                      <td
                        colSpan={3}
                        className="px-3 py-4 text-ink-soft"
                      >
